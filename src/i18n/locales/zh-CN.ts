...
export const zhCN: Dict = {
  ...
  'settings.welcomeSubtitle': '选择你希望使用的执行方式。后续可以随时从顶部「设置」按钮中修改。',
  'settings.kicker': '设置',
  'settings.title': '执行模式与模型',
  'settings.subtitle':
    '在本机的代码代理 CLI 与 HTTP API（Anthropic 自带 Key，或 LM Studio 等 OpenAI 兼容本机服务）之间切换。凭据只保存在当前浏览器。',
  ...
  'settings.modeApiMeta': 'Anthropic 或本机模型',
  ...
  'settings.apiHint': 'Anthropic：请求由当前浏览器直连你设置的 Base URL，Key 只存放在 localStorage。',
  'settings.apiProviderAria': 'API 提供方',
  'settings.providerAnthropic': 'Anthropic',
  'settings.providerAnthropicMeta': 'Messages API · 自带 Key',
  'settings.providerOpenAiLocal': 'OpenAI 兼容',
  'settings.providerOpenAiLocalMeta': 'LM Studio、llama.cpp 等',
  'settings.apiHintOpenAiLocal':
    '请求由本机的 Open Design 守护进程转发（同源），避免浏览器访问 localhost 时的 CORS 问题。请先启动 LM Studio（或你的服务），并用带守护进程的方式运行应用（例如 pnpm dev:all）。若服务不要求鉴权，可不填 API Key。',
  'settings.placeholderApiKeyAnthropic': 'sk-ant-api03-…',
  'settings.placeholderApiKeyOptional': '可选 — 不需要可留空',
  'settings.localOpenAiApi': '本机 LLM',
  'settings.lmStudioCardTitle': 'LM Studio',
  'settings.lmStudioCardMeta': '本机 OpenAI 兼容 API · 默认 :1234',
  ...
  'chat.startHint': '可以拖拽或粘贴图片作为视觉参考，或键入 @ 引用本项目中的文件。也可以从下面的示例开始：',
  ...
  'chat.localLlmNeedsDaemon':
    '本机 OpenAI 兼容对话需要 Open Design 守护进程（由它转发到 LLM 服务）。请先通过 pnpm dev:all 或 od CLI 启动守护进程后再试。',
  'chat.stop': '停止',
  ...
  'agentPicker.byok': 'API · 自带 Key / 本机',
